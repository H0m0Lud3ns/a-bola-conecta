...
const isAffirmative = (value) => ['sim', 'true', '1', 'yes', 'quero', 'solicito'].includes(clean(value, 80).toLowerCase());
const sheetCell = (value, max = 2000) => {
  const cell = clean(value, max);
...
const classifyLeadType = (formId, fields = {}) => {
  ...
  if (id.includes('apoie') || id.includes('apoio') || id.includes('pix') || hasValue(fields.valor) || hasValue(fields.valor_pix) || hasValue(fields.valorOutro) || hasValue(fields.valor_outro) || hasValue(fields.dataPix) || hasValue(fields.data_pix)) return 'support_pix';
...
const normalizeLead = (body, req) => {
  ...
  const messageParts = [
    ...
    fields.valor || fields.valor_pix ? `valor_pix=${fields.valor || fields.valor_pix}` : '',
    fields.valor_outro || fields.valorOutro ? `valor_outro=${fields.valor_outro || fields.valorOutro}` : '',
    fields.dataPix || fields.data_pix ? `data_pix=${fields.dataPix || fields.data_pix}` : '',
    fields.pix_end_to_end ? `pix_end_to_end=${fields.pix_end_to_end}` : '',
    fields.nota_fiscal ? `nota_fiscal=${fields.nota_fiscal}` : '',
  ...
  const contactRecord = {
    ...
    city: clean(fields.cidade || fields.city || fields.comuna || fields.cidade_pais, 180),
  ...
  return {
    ...
    support_pix: leadType === 'support_pix' ? {
      value_informed: clean(fields.valor || fields.valor_pix || fields.valor_informado, 80),
      ...
      pix_end_to_end: clean(fields.pix_end_to_end || fields.end_to_end || fields.identificador_pix, 180),
      purpose: clean(fields.finalidade || fields.participacao || fields.interesse || 'apoio-circulacao-educativa', 220),
      contribution_model: clean(fields.modelo_contribuicao || 'valor_livre_acesso_aberto', 160),
      fiscal_document: clean(fields.cpf_cnpj || fields.cpf || fields.cnpj, 80),
      invoice_requested: true,
      invoice_status: clean(fields.cpf_cnpj || fields.cpf || fields.cnpj, 80) ? 'pendente_emissao_nfse' : 'pendente_nfse_cliente_tomador_nao_informado',
      reconciliation_status: 'recebido_formulario_pendente_extrato',
      bank_statement_status: 'pendente_conciliacao_extrato_diario',
      accountability_method: 'formulario_pix_extrato_diario',
    } : null,
    ...
    compliance: {
      ...
      notes: leadType === 'support_pix' ? 'Registrar como receita tributavel de servico educativo em conta PJ. Conciliar Pix por extrato diario e emitir NFS-e para cada recebimento. Se CPF/CNPJ nao for informado, emitir com cliente/tomador nao informado conforme orientacao Contabilizei.' : '',
    },
    tracking,
    raw_fields: fields,
...
const leadToSheetRow = (lead) => [[
  ...
  lead.lead_type,
  lead.form_id,
  lead.support_pix?.value_informed || '',
  lead.support_pix?.custom_value || '',
  lead.support_pix?.pix_date_informed || '',
  lead.support_pix?.pix_name_informed || '',
  lead.support_pix?.pix_end_to_end || '',
  lead.support_pix?.purpose || '',
  lead.support_pix?.contribution_model || '',
  lead.support_pix?.invoice_requested ? 'sim' : 'nao',
  lead.support_pix?.fiscal_document || '',
  lead.support_pix?.reconciliation_status || '',
  lead.support_pix?.invoice_status || '',
  lead.support_pix?.accountability_method || '',
  lead.compliance?.notes || '',
  lead.owner,
  lead.priority,
  lead.user_agent,
]].map((row) => row.map((value) => sheetCell(value, 2000)));
...
const appendToGoogleSheets = async (lead, config = destinationConfig()) => {
  ...
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${config.googleSheetsId}/values/${range}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`;
  const response = await fetch(url, {
    method: 'POST',
    headers: {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const criticalFailures = destinations.filter((destination) => !destination.ok && ['crm', 'webhook', 'sheets', 'google_sheets'].includes(destination.name));
    if (criticalFailures.length) {
      return json(res, 502, { ok: false, dry_run: false, error: 'destination_failed', destinations });
    }
    return json(res, 200, { ok: true, dry_run: false, lead_type: lead.lead_type, form_id: lead.form_id, destinations });
  } catch (error) {
