...
(function () {
  ...
  var MAP = [
    { match: 'solu\u00e7\u00f5es educativas', href: '/servicos/', key: 'educator' },
    { match: 'propostas B2B',                  href: '/contato/',  key: 'company'   },
    { match: 'Apoiar agora',                  href: '/comunidade/', key: 'supporter' }
  ];
  ...
  function applyOverlay() {
    var found = Array.from(document.querySelectorAll('*')).filter(function (el) {
      ...
      return direct && MAP.some(function (m) { return text.indexOf(m.match) >= 0; });
    });

    found.forEach(function (el) {
      var text = (el.textContent || '').trim();
      var entry = MAP.find(function (m) { return text.indexOf(m.match) >= 0; });
      if (!entry) return;
